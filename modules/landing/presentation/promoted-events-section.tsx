...
export const PromotedEventsSection = () => {
  ...
  const events = ctx.usePromotedEvents();
  const isPromotedEventsLoading = ctx.useIsPromotedEventsLoading();

  useEffect(() => {
    ctx.loadPromotedEvents();
  }, [ctx]);

  if (!isPromotedEventsLoading && events.length === 0) {
    return null;
  }
